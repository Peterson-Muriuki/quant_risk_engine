...
from engine import fit_yield_curve, nelson_siegel, calculate_risk_metrics
...
# Sample Treasury Data (1mo, 3mo, 6mo, 1yr, 2yr, 5yr, 10yr, 30yr)
...
if not data.empty:
    # Handle yfinance multi-index/multi-column returns
    try:
        # Select 'Close' column and flatten if it's a DataFrame
...
col1.metric("Market Sentiment", sentiment_label)
col2.metric("Sentiment Score (NLP)", f"{score:.2f}")

st.info(f"The NLP Engine (M5) has analyzed recent headlines for {ticker}. "
        f"A score > 0 indicates positive social/news momentum.")